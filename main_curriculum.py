...
from curriculum_utils import ProteinDatasetLoader
...
class MRNDesignCurriculum:
    ...
    def __init__(self, tasks, curriculum_config=None):
        ...
        self.config = {
            'lpe': 'Online',           # More stable than Linreg
            'acp': 'LP',               # Simpler than MR
            'a2d': 'GreedyProp',       # More stable than Boltzmann
            'a2d_eps': 0.15,           # Add exploration
            'lpe_alpha': 0.05,         # Slower updates
            'acp_MR_K': 25,            # Larger window for stability
            'acp_MR_power': 2,         # Lower power (less aggressive)
        ...
        self.compute_dist = make_dist_computer(
            self.num_tasks,
            lpe=self.config['lpe'],
            lpe_alpha=self.config['lpe_alpha'],
            acp=self.config['acp'],
            acp_MR_G=self.G,
    ...
    while global_step < total_steps:
        ...
        if global_step % eval_every == 0:
                ...
                for task_idx, task in enumerate(curriculum.tasks):
                    ...
                    performance = evaluate_task_performance(eval_env, sampler, n_samples=16)
...
def evaluate_task_performance(env, sampler, n_samples=32):
...
def main():
    ...
    # Training parameters : 10 Global Steps, at each step, we sample a task, we train the model for 200 steps, we evaluate the model every 5 steps
    ...
    parser.add_argument('--run_name', type=str, default='Curriculum_GFN_25_to_180')
