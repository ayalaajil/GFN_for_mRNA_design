...
SIMPLE_PAIRING_FE = dict(base_pair_AU=-1.6, base_pair_CG=-3.0,
                         base_pair_GU=-0.4, hairpin_01=1)
...
class RNASolutionBase:
    ...
    def __init__(self, rna: str):
        ...
        self.irna = np.fromiter(map(base_as_int, rna.upper()),
...
class RNAFolderBase:
    ...
    def __init__(self, loop_min=0, energies=None):
        ...
        self._internal_fe, self._bulge_fe, self._hairpin_fe = \
            self._loop_lookups(energies)
    ...
    @staticmethod
    def _interpolated_table(energies: dict, size: int, key: str,
                            fmt: str = "02d") -> np.ndarray:
    ...
    def _loop_lookups(energies: dict):
        ...
        i = RNAFolderBase._interpolated_table(energies, RNAFolderBase.MAX_INTERNAL + 1,
                                              "internal_")
        b = RNAFolderBase._interpolated_table(energies, RNAFolderBase.MAX_BULGE + 1,
                                              "bulge_")
        h = RNAFolderBase._interpolated_table(energies, RNAFolderBase.MAX_HAIRPIN + 1,
                                              "hairpin_")
        return i, b, h
    ...
    def print_term_fe(self, rp: str, an: str, bn: str):
        ...
        print(f"Free energy closing with {rp} a loop {an}...{bn} (helix_terminal_{rp}{an}{bn}) ={e:6.1f}")

    def print_hairpin_fe(self, n: int):
        e = self._hairpin_fe[n]
        print(f"Energy penalty when closing a hairpin loop of size {n} (hairpin_{n:02d}) ={e:6.1f}")
...
def print_workings(s: RNASolution):
    ...
    for i in range(N):
        ...
        for j in range(N):
            if j < i:
                u = s.getU(j, i)
                print("    . " if u == 0. else f"{u:6.1f}", end='')
            elif j > i:
                k = s.getK(i, j)
